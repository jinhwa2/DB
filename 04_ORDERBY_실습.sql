/*ORDER BY 실습하기*/
--1.EMPLOYEE테이블에서 모든 직원의 이름을 오름차순 정렬
--모두 보기
SELECT *
FROM EMPLOYEE
ORDER BY EMP_NAME ASC ;

--2.EMPLOYEE 테이블에서 모든직원의 급여를 내림차순으로 정렬 조회
--모두 보기
SELECT *
FROM EMPLOYEE
...
--> 사람이나 어떤 값을 정렬해서 모두 보겠다X
--> 제일 많이 보너스 받는 값은 무엇이고 월급 낮게 받거나 높게 받는 금액 기분

--6.EMPLOYEE테이블에서 모든 직원의 이메일을 '이메일'표기
--EMAIL 오름차순 정렬
--전화번호를 AS "전화번호' 표기
--EMAIL 내에서는 전화번호를 내림차순으로 표기
