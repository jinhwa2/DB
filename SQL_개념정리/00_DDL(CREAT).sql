...
VARCHAR2 : 최신버전 제일 많이 사용
    ...
    CHAR     VS   VARCHAQR2
    고정             가변
   예를들어 CHAR(10BYTE) VARCHAQR2(10BYTE)
                CHAR = 1;  ->1하나를 넣는데 10을 모두 사용
            VARCHAQR2 = 1; ->1하나를 넣으면 나머지 99를 돌려줌
            

VARCHAR2 같은 경우 공간 확보는 하되, 사용하지 않은 크기는 돌려줌

DATE : 날짜 타입
CLOB : 메일,항소문,소문 등 VARCHAR2에서 감당하지 못할 크기의
       글 내용을 삽입할 때 사용
...
--테이블 만들 때 다른 테이블과 동일하게(컬럼,데이터값)복사 붙여넣기를 진행할 때는
SELECT *FROM EMPLOYEE2;
