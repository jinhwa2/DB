...
회원가입할때 DB에 들어오는 현재시간을 기록할 때 생성
...
TO_DATE("문자열","패턴")
예를들어 2024-05-01 자 이후 회원가입 한 회원 조회
...
지정된 컬럼의 값이 중복되는 경우
중복되는 값을 한번만 표시할 때 사용

사용방법
SELECT DISTINCT 중복제거할 컬럼명
FROM 테이블명
*/
...
--EMPLOYEE 테이블에서 D9 부서에 해당하는 사원의 이르 조회
SELECT EMP_NAME
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
컬럼명 IN(값1,값2,값3,...)
--컬럼의 값이 () 내값과 일치하면 TRUE

컬럼명 NOT IN(값1,값2,값3,...)
--컬럼의 값이 () 내값과 일치하면 FALSE
--컬럼의 값이 () 내값과 일치하지 않으면 TRUE
*/
...
--EMPLOYEE에서 성이 전씨인 사원의 사번 이름 조회  WHERE 컬럼명 LIKE '패턴'
...
ESCAPE '#'구분을 지을 것이다.
___LIKE 사용하는 3글자만 찾으라는 의미로 구분 짓는 것
_% ->_로된 글자 찾기
...
--->나타난 정보를 오름차순으로 정렬할지 내림차순으로 정럴할지
---->정렬만 하기 때문에 별칭으로 사용할 수 있음

--EMPLOYEE 테이블에서 이름, 부서코드, 급여를
--부서코드 오름차순, 급여 내림차순 조회
SELECT EMP_NAME,DEPT_ CODE,SALARY
FROM EMPLOYEE
ORDER BY DEPT_ CODE ASC  SALARY DESC;

--EMPLOYEE 테이블에서 이름,부서코드,직급코드(JPB_CODE)를
...
--EMP_NAME : 현재2번째 자리에 위치하는 순서를 이용해서 ORDER BY를 구할수 있음
SELECT EMP_ID,EMP_NAME,SALARY
FROM EMPLOYEE
WHERE SALARY BETWEEN 3000000 AND 6000000
ORDER BY 2 DESC; --2 = EMP_NAME
--만약에 EMP_ID로 ORDER BY에서 정렬하길 원한다면 1을 작성
--만약에 SALARY로 ORDER BY에서 정렬하길 원한다면 3을 작성
...
그다음 J7은 단독이기 때문에 정렬할 수 있는게 없음
J6에서는 2명 이상이 존재하기 때문에 J6 직급으 직원들의 이름이 오름차순으로 정렬
