...
단일 행 함수 : N개의 행 의 컬럼 값을 전달하여 N개의 결과가 반환

그룹 함수 : N개의 행 의 컬럼 값을 전달하여 1개의 결과가 반환
         (그룹의 수가 늘어나면 그룹의 수만큼 결과를 반환)
함수는 SELECT절, WHERE절, ORDER BY절, GROUPT BY절,HAVING절에서 사용가능

SELECT절 : 찾는 기준이 아닌, 눈으로 표기되는 총합,계산 결과 등에 대한 것을 눈으로 봤을 때
          정리해놓은 컬럼
          -> 정리해서 보여주자!
          
WHERE 절 : 찾는 조건이 되는 행을 작성
...
SELECT 'HELLO WORD',LENGTH('HELLO WORLD')
FROM DUAL;
...
FROM EMPLOYEE
WHERE LENGTH(EMAIL) <= 12 --12길이 보다 이하인 이메일만 조회
ORDER BY "이메일길이";

--INSTR(문자열 | 컬럼명,'찾을 문자열'[,찾는 것을 시작할 위치[,순번 ] )

--찾을 시작 위치부터 지정된 순번째 찾은 문자열의 시작위치를 반환
...
--AAB AAC AA B 13번째 자리에서 B가 두 번째로 나타났기 때문에 B는 13번째 자리에 위치한다 표시된 것

--SUBSTR(문자열 | 컬럼명,시작위치[,길이])
--문자열을 시작위치부터 지정된 길이만큼 잘라내서 반환
--길이를 작성하지 않은 경우 시작위치부터 끝까지 잘라내서 반환
...
--찾음다음에 @가 표시되면 안되기 때문에 -1설정을해서 @제거
...
SELECT FLOOR(5.789,1) FROM DUAL; --에러발생 소수표현 X

----- 2) TRUNC : 숫자의 소수점을 잘라내는 역할
---------자르고자 하는 자릿수를 지정 지정하지 않으면 기본값0
...
//--앞쪽 뒤쪽 양쪽 공백제거 한번에 사용하기 (주석 컨트럴 슬래시 단축키)
SELECT '     점         심    ',
        TRIM(LEADING ' ' FROM'     점         심    ') --앞쪽 공백제거
        TRIM(TRAILING ' ' FROM'     점         심    ')--뒤쪽 공백 제거
        TRIM(BOTH ' ' FROM'     점         심    ') --양쪽 공백제거
FROM DUAL;

/*
REPLACE : 특정 문자열을 대체할때 사용
...
SELECT NATIONAL_CODE , NATIONAL_NAME, REPLACE(NATION_NAME,'한국','대한민국')
FROM NATIONAL;
...
추후에 java int 값이 아니라 String 값으로 넘어오고
