...
1  FROM 참조할 테이블명
2. WYERE 컬럼명 | 함수식 비교연산자 비교값
3. GROP BY 그룹을 묶을 컬럼명
4. HAVING 그룹함수식 비교연산자 비교값
6. ORDER BY 컬럼명 | 별칭 | 컬럼순서 정렬방식
...
그룸함수는 단 한개의 결과 값만 산추하기 때문에 그룹이 여러개일 경우 오류
...
--1.SELECT 절에서 부서코드랑 보너스사원수 카운트진행
...
--EMPLOYEE 테이블에서 부서마다 D5 D6인 부서의 평균 급여 조회
--버림 평균 FLOOR AVG 급여
...
-->급여가 3000000원 이상인 직원들 대상으로 각 부서의 평균급여 계산
---> 그룹화 전에 필터링 일어남
...
--부서별 평균 급여가 3000000원 이상인 부서를 조회하고 부서코드 오름차순
-->모든 직원들을 대상으로 그룹화한 후, 각 부서별로 편균급여를 계산
...
-- WHERE ORDER SELECT 에서 사용하지 않고 GROUP BY에서 작성
...
/*
--> employee 테이블에서 DEPT_CODE와 JOB_CODE기준으로 SALARY  합계 구함
소계 : 특정그룹의 합계 전체 
부서코드가 동일한 모든 행의 합계
모든 합계
그룹의 합계
...
/*
CUBE는 ROLL UP 보다 더 포괄적인 집게 결과 생성
CUBE는 가능한 모든 것을 조합하고 조합에 대한 결과와 총합 계산
부서 직급   SUM(SALARY)
D1	J6	6440000  <-- D1 부서의 J6 직급 급여 합계
D1	J7	1380000  <-- D1 부서의 J7 직급 급여 합계
D1	총합 7820000 <-- D1 부서 전체의 급여 함계(소계)
...
D8	J6	6986240    <-- D8 부서의 J6 직급 급여 합계
D8	총합 6986240    <-- D8 부서의 전체(J6) 급여합계(소계)
D9	 J1	  8000000    <-- D8 부서의 J6 직급 급여 합계(소계
D9	 J2	  9700000    <-- D8 부서의 J1 직급 급여 합계
D9	 null 17700000     <-- D8 부서의 J1 직급 급여 합계
...
null	J6	2320000    <-- 모든 부서의 J6 직급 급여 합계
...
----MINUS : 선행 SELCET 결과에서 다음 SELECT 결과와 겹치는 부분을 제외한 나머지 부분만 추출---
---부서코드가 D5중 급여가 300만원 초과인 직원 제외
--1)부서코드가 'D5'
SELECT EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
...
--주의할점 : 집합 연산에 사용되는 SELECT문의 SELET절이 동일해야함

--UNION은    OR 같은 개념 --> 중복제거
--INTERSECT  AND 같은 개념
--MINUS는    차집합 같은 개념
--UNION ASS 은 OR 결과 값에 AND 결과 같이 더해진 값 --> 중복에 제거되지 않은 채 합쳐짐
