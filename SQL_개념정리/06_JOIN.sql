/*
[JOIN]
- 하나 이상의 테이블에서 데이트를 조회하기 위해 사용
- 수행결과는 하나의 RESULT SET 으로 나옴

- 관계형 데이터베이스에서 SQL 이용해 테이블간 관계를 맺는 방법

- 관계형 데이터 베이스틑 최소한의 데이트를 테이블에 담고 있어
  원하는 정보를 테이블에서 조회하려면 한 개 이상의 테이블에서
  데이트를 읽어와야 하는 경우가 많음
   이 때, 테이블과 관계를 맺기 위한 연결고리 역할이 필요한데
...
/*
EMPLOYEE 테이블과 department  테이블 조인해서
직원ID, 이름, 부서코드, 부서ID, 부서명을 조회하는 쿼리

JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);

//JOIN은 단순히 테이블 두 개 붙이는 것이 아닌
...
--DEPARTMENT 테이블, LACATION 테이블 참조해서 부서명, 지역명 조회

SELECT *FROM DEPARTMENT;

SELECT *FROM LOCATION;
--ANSI 방식 JOIN ON LOCATION_ID = LOCAL_CODE
SELECT DEPT_TITLE AS "부서명",LACAL_CODE AS "코드명",LOCAL_NAME AS "지역명"
FROM department
JOIN location ON (LOCATION_ID = LOCAL_CODE);

--ORCLE 방식 LACATION_ID = LOCAL_CODE
