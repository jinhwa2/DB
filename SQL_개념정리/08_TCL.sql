...
변경된 사항을 묶어서 하나의 트랙잭션에 처리

트랜잭션에서 데이터 변경 대상 : INSERT, UPDATE, DELETE, (DML)

EX) 
     INSERT INTO 값 추가 ---------------------------------> DB 반영
     INSERT INTO 값 추가 ----트랜잭션에 추가------COMMIT----> DB 반영
     INSERT INTO 10번 값 추가 --> 1개 트랙젝션에 10개 추가--ROLLBACK --->DB 반영(X)
...
ROLLBACK; -----> 완벽하게 저장이 된 상태에서 돌아가기 진행할 경우
----------------> 커릿이 완료된 시점으로 돌아감 
--48~51 사이나 다른 구문에서 INSERT UPDATE DELETE 해준 사항이 없기 때문에 변경X
...
SELECT *FROM department3;  --> 실수로 전체내역 삭제


--AAA 까지 롤백 (AAA까지 돌아가고 싶다)
ROLLBACK TO "AAA";
