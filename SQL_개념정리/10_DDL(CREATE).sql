...
-- 테이블로 생성된 객체는 DROP 구분을 통해 제거할 수 있음

1. 테이블 생성하기
- 테이블이란?
-- 행(ROW)과 열(COLUM)으로 구성되는 가장 기본적인 데이터베이스 객체
...
CREATE TABLE "USER_MEMBER"(USER_ID VARCHAR2(20),USER_PW VARCHAR2(20),USER_NAME VARCHAR2(15),
                                               USER_SSN  CHAR(14));
                                               
                                               
    CREATE TABLE "USER_MEMBER"(USER_ID VARCHAR2(20),USER_PW VARCHAR2(20),USER_NAME VARCHAR2(15),
                                               USER_SSN  CHAR(14),ENROLL_DATE DATE DEFAULT SYSDATE);   
...
-- 가입일 : ENROLL_DATE                          ---DEFAULT SYSDATE --현재시간을 기본값으로 고정
...
CREATE TABLE UNIQUE_TABLE (
      ...
      --USER_ID VARCHAR2(20) UNIQUE, --컬럼 레벨 베약조건 (이름 X)
      --(방법1)
      USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, 
      --CONSTRANIT 제약조건명 제약조건 종류
      ...
      --테이블 레벨--
      --UNIQUE(USER_NAME) --컬럼 레베 ㄹ제약 조건(이름X)
...
INSERT INTO KH_CAFE
    VALUES(1,'맥아커피','02-1111-11111','서울시 강남구 광남동2길');
...
--  (방법1)
    컬럼명 자료형(정수자리값, 실수 자리값) PRIMARY KEY,


--  (방법2)
     컬럼명 자료형(정수자리값, 실수 자리값)  CONSTRAINT 인덱스에저장할이름명 PRIMARY KEY


--  (방법3)
     컬럼명 자료형(정수자리값, 실수 자리값)
      CONSTRAINT 인덱스에 저장할이름명 PRIMARY KEY(연결할컬럼명)
    );
*/
CREATE TABLE USER_PRIMARY_TABLE (
...
--UNIQUE 와 PRIMARY의 차이점
--UNIQUE는 NUL 한개는 허용됨
--PRIMARY KEY NULL값 안되고 중복 안됨
...
-- USER GRADE GRADE_CODE 참조하는 테이블 생성
...
INSERT INTO USER_FK VALUES (2,'USER01','PW01',20);

INSERT INTO USER_FK VALUES (3,'USER03','PW03',30);
INSERT INTO USER_FK VALUES (4,'USER03','PW04',40);

--paren key not found 부모인 키 USER_GRDE에 40이라는 값이 없어서 넣을 수 없음
...
-->핸드폰번호를 기입할 때는 VARCHAR2를 사용해서 010모두 들어갈 수 있도록 설정
...
-USE-GRDE GRADE_CODE 참조하는 테이블 생성
...
(방법2)- 부모키 삭제시 자식키를  NULL로 변경하는 옵션ON DELETE SET NULL
