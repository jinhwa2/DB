...
--데이터정의 언어로 객체를 만들고(create) 수정하고(alter) 삭제(drop) 구문
...
-- CONSTRAINT 제약조건 이름 DEPT_DOPY_UNI

ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_DOPY_UNI UNIQUE(DEPT_TITLE);

-- 2. 컬럼 추가 / 수정 / 삭제

-- 컬럼추가 : ALTER_TABLE 테이블명 ADD(컬럼명 데이터타입 [DEFAULT] '값']);

-- 컬럼 수정 : ALTER_TABLE 테이블명 MODIFY 컬럼명 데이터타입 : (데이터 타입 변경)
-->               ALTER_TABLE 테이블명 MODIFY DEFAULT '값':(기본값 변경)

---> 데이터 타입 수정할 경우 컬럼에 저장된 크기 미만으로는 변경할 수 없음
----------> 데이터타입 VARCHAR2(100) -> VARCHAR2(10) 데이터 티입이 10 초과되는건 불가

-- 컬럼 삭제 : ALTER TABLE 테이블명 DROP (삭제할컬럼명);
-->               ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;

--> 테이블에서는 최소1개 이상의 컬럼이 존재해야하기 때문에 모든 컬럼 삭제 X

-- DEPT_COPY 컬럼에 CNAME VARCAHR2(20) 컬럼 추가
...
--외래 키에 의해 참조되는 고유 / 기본 키가 존재하기 때문에 삭제 할 수 없음
-- 삭제하고 싶은데 어떻게 삭제를 하는가 ?

-- 해결방법1 : 자식테이블(TB2) 삭제 후 부모테이블(TB1) 삭제
DROP TABLE TB2;
DROP TABLE TB1;

-- 해결방법2 : CACADE CONSTRAINTS 옵션 사용
