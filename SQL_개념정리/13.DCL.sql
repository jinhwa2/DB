/*계정(사용자)
 *관리자계정 : 데이터 베이스의 생성과 관리를 담당하는 계정
                    모든 권한과 책임을 가지는 계정
                    //
 
 
 
 *사용자계정: 데이터베이스에 대해 질의, 갱신, 보고서 작성등의
                    작업을 수행할 수 있는 계정으로
 ...
 *권한의 종류
 ...
 1) 시스템 권한 : DB접속, 객체생성권한
 CREATE USER : 사용자(계정) 생성 권한
 CREATE TABLE : 테이블 생성 권한
 CREATE SESSTION : 데이터베잉스 접속 권한
 CREATE VIEW : 뷰 생성권한
 CREATE SEQUENCE : 시퀀스 생성 권한
 CREATE PROCECURE : 함수(프로시저)생성 권한
...
계정에 권한을 부여할 때 하나만 부여할 경우 (,사용해서 작성)
GRANT 권한, 권한 TO 계정명;
 
 대표적
 CONNECT : 사용자 데이터베이스에 접속 가능하도록 하기 위해 설정
                    -> 권한 8가지 묶음 패키지
                    SESSION(DREATE ALTER), CLUSTER,LINK,SEQUENCE,SYNONYM,TABLE,VIEW
 
 RESOURCE : 사용자가 객체(테이블,뷰,인덱스)를 생성할 수 있도록 하기위해 시스템 권한을 묶어 놓음
                    ->권한 5가지 묶음 패키지
