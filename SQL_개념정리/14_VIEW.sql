...
-- KH_WOUKBOOK 뷰를 생성할 권한이 부족하다는 의미
이 문제를 해결하려면 데이터 베이스 관리자(DBA)나 시스템 관리자한테 권한을 요청

GRANT CREAT VIEW TO KH_WOUKBOOK;
VIEW 를 생성할 권한 부여

View ALL_COMPANIES이(가) 생성되었습니다.

VIEW  는 주로 테이블을 복제하거나 조합해서 테스트용 SELECT 많이 사용
SELECT 문 저장하고 다시보기로 사용
*/

--가격이 1500 이상인 과자를 조회하고 나중에 또 다시 볼 수 있는 뷰 생성
--> VIEW ONEFIVE_SNACKS

CREATE VIEW SNAKDS_IN AS SELECT * FROM SNACKS WHERE PRICE >= 1500;
