...
SELECT AVG(SALARY) 
FROM EMPLOYEE E
WHERE DEPT_CODE = E.DEPT_CODE; --의미가 없는 코드X

SELECT AVG(SALARY) 
FROM EMPLOYEE E;   --문제가 없음
...
--3.D5 부서의 평균 급여보다 높은 급여를 받는 직원 들의 이름 조회
--SELECT  EMP_NAME            AVG
SELECT EMP_NAME,SALARY
FROM EMPLOYEE
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE);

--4. 급여가 300만원 이상인 직원중에서 가장 낮은 급여를 받는 직원의 이름과 급여조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
...
WHERE SALARY >= 300000; --> 급여가 300만원 이상인 행들만 최소값을 계산
