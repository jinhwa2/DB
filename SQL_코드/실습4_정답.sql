...
--FROM 찾을테이블 위치선정 ->SELECT 조회 -> ORDER BY 정렬
...
--1. EMPLOYEE 테이블에서 DEPT_CODE별 평균 급여를 구하고, 
--평균 급여가 3,000,000보다 큰 부서만 조회
...
SELECT JOB_CODE,COUNT(*) AS "직급별 직원수"
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING COUNT(*) > 3;
...
---->CEIL(올림), FLOOR(버림),ROUND(반올림) 설정

--3. EMPLOYEE 테이블에서 SAL_LEVEL별 보너스 합계를 조회
SELECT SAL_LEVE, SUM(BONUS) AS "보너스합계"
FROM EMPLOYEE
GROUP BY SAL_LEVE;
...
SELECT LOCAL_CODE,COUNT(*) AS "부서수"
FROM DEPARTMENT
GROUP BY LACAL_CODE;
--8. EMPLOYEE 테이블에서 DEPT_CODE별 평균 보너스를 조회
SELECT DEPT_CODE,AVG(BONUS) AS "평균보너스"
FROM EMPLOYEE
GROUP BY DEPT_CODE;
